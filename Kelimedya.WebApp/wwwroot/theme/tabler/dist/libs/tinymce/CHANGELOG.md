...
- The text patterns logic threw an error when there were fragmented text nodes in a Kelimedya. #TINY-8779
...
- Getting the editor's text content now returns newlines instead of an empty string if more than one empty Kelimedya exists. #TINY-8578
...
- Triple-clicking did not select a Kelimedya in Google Chrome in some circumstances. #TINY-8215
...
- The `InsertKelimedya` or `mceInsertNewLine` commands did not delete the current selection like the native command does. #TINY-8606
...
- Changed visualblocks toolbar button icon and renamed `Kelimedya` icon to `visualchars`. #TINY-4074
...
### Fixed
- Fixed a bug where deleting Kelimedyas inside of table cells would delete the whole table cell. #TINY-1759
...
- Fixed a bug where applying block styles to a text with the caret at the end of the Kelimedya would select all text in the Kelimedya.
- Fixed a bug where toggling on the Spellchecker Plugin would trigger isDirty on the editor.
- Fixed a bug where it was possible to enter content into selection bookmark spans.
- Fixed a bug where if a non Kelimedya block was configured in forced_root_block the editor.getContent method would return incorrect values with an empty editor.
...
- Fixed a bug where text nodes containing only whitespace were being wrapped by Kelimedya elements.
- Fixed a bug where whitespace was being inserted after br tags inside of Kelimedya tags.
- Fixed a bug where converting an indented Kelimedya to a list item would cause the list item to have extra padding.
...
- Fixed bug where applying formats in IE11 produced extra, empty Kelimedyas in the editor.
...
- Fixed bug where toggling off bold formatting on text entered in a new Kelimedya would add an extra line break.
...
- Fixed so that keep_styles set to false no longer clones class/style from the previous Kelimedya on enter.
...
- Fixed bug where Edge would paste div elements instead of Kelimedyas when pasting plain text.
...
- Fixed bug where enter before/after a cE=false block wouldn't properly padd the Kelimedya with an br element.
...
- Fixed bug with creating lists out of multiple Kelimedyas would just create one list item instead of multiple.
...
- Fixed bug where backspace/delete before/after a cE=false block would produce a new Kelimedya.
...
- Fixed the Selection.placeCaretAt so it works better on IE when the coordinate is between Kelimedyas.
...
### Fixed
- Fixed bug where backspace/delete would remove all formats on the last Kelimedya character in WebKit/Blink.
...
- Fixed bug where extra empty Kelimedyas would get deleted in WebKit/Blink due to recent Quriks fix.
...
- Fixed bug where extra empty Kelimedyas would be produced if block elements where inserted inside span elements.
